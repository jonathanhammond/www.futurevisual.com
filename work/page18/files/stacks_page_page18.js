...
// Javascript for stacks_in_3152_page18
// ---------------------------------------------------------------------

// Each stack has its own object with its own namespace.  The name of
// that object is the same as the stack's id.
stacks.stacks_in_3152_page18 = {};

// A closure is defined and assigned to the stack's object.  The object
// is also passed in as 'stack' which gives you a shorthand for referring
// to this object from elsewhere.
stacks.stacks_in_3152_page18 = (function(stack) {
 ...
 var opacity=100,opacityBottom=85,hoverOpacity=15,subNavIndex=1,divide=0,zoneDivider=0,fullWidthMenu='full-width',scope='all',style='solid',menuAlign='right',baseColor='#FFFFFF',baseColorTop='#737373',hoverColor='#FFFFFF',styleActive='styleActive'==='styleActive'?true:false,rwmenu='rw'==='rw'?true:false,magellan='rw'==='magellan'?true:false,menuItems='#stacks_in_3152_page18 ul:not(.title-area) li:hover:not(.has-dropdown) > a:not(.button)',dropdowns='#stacks_in_3152_page18 ul:not(.title-area) li.has-dropdown';if(opacity<100||style=="faded"){$.glass({colors:[baseColor,baseColorTop],gradient:style,opacity:[opacity,opacityBottom],selector:'#stacks_in_3152_page18 .top-bar-wrapper'});$.glass({colors:[baseColor],opacity:[opacity],selector:'#stacks_in_3152_page18 ul:not(.title-area) ul.dropdown'});}
if(hoverOpacity<100){if(styleActive){var active=magellan?'#stacks_in_3152_page18 .top-bar-wrapper.styleActive .magellanList li.active > a':'#stacks_in_3152_page18 .top-bar-wrapper.styleActive li.active > a';$.glass({colors:[hoverColor],opacity:[hoverOpacity],selector:active});}
...
// Javascript for stacks_in_1972_page18
// ---------------------------------------------------------------------

// Each stack has its own object with its own namespace.  The name of
// that object is the same as the stack's id.
stacks.stacks_in_1972_page18 = {};

// A closure is defined and assigned to the stack's object.  The object
// is also passed in as 'stack' which gives you a shorthand for referring
// to this object from elsewhere.
stacks.stacks_in_1972_page18 = (function(stack) {
...
(function(b){var a=function(d){var c=b(this);if(c.data("resizetimer")){window.clearTimeout(c.data("resizetimer"))}c.data("resizetimer",window.setTimeout(function(){c.trigger("afterresize")},300))};b.event.special.afterresize={add:function(c){b(this).bind("resize",a);var d=c.handler;c.handler=function(e){return d.apply(this,arguments)}},remove:function(c){b(this).unbind("resize",a)}};b.fn.extend({afterresize:function(c){if(b.isFunction(c)){b(this).bind("afterresize",c)}else{b(this).trigger("afterresize")}return this}})})(jQuery);$(window).load(function(){function build_mask_stacks_in_1972_page18(){var box_width=$('#stacks_in_1972_page18 .boxgrid').width();var box_height=300;var back_height=$('#stacks_in_1972_page18 .boxgrid .back').height();var cover_height=$('#stacks_in_1972_page18 .boxgrid .cover').height();box_height=back_height;if(back_height>cover_height){$('#stacks_in_1972_page18 .boxgrid,#stacks_in_1972_page18 .boxgrid .cover').height(back_height);}
else{box_height=cover_height;$('#stacks_in_1972_page18 .boxgrid,#stacks_in_1972_page18 .boxgrid .back').height(cover_height);}}
build_mask_stacks_in_1972_page18();if(typeof(orientationEvent)==undefined){var orientationEvent=0;}
$(window).bind(orientationEvent,build_mask_stacks_in_1972_page18).bind('resize',build_mask_stacks_in_1972_page18).afterresize(build_mask_stacks_in_1972_page18);});

	return stack;
})(stacks.stacks_in_1972_page18);


// Javascript for stacks_in_21595_page18
// ---------------------------------------------------------------------

// Each stack has its own object with its own namespace.  The name of
// that object is the same as the stack's id.
stacks.stacks_in_21595_page18 = {};

// A closure is defined and assigned to the stack's object.  The object
// is also passed in as 'stack' which gives you a shorthand for referring
// to this object from elsewhere.
stacks.stacks_in_21595_page18 = (function(stack) {
 ...
 $(document).ready(function(){var stack=$('#stacks_in_21595_page18');$('ul',stack).first().addClass('side-nav custom').removeClass('dropdown');$('ul li',stack).each(function(){if($(this).find('ul').length==0)$(this).removeClass('has-dropdown');});$('ul.side-nav li a header',stack).unwrap().parent().addClass('heading');$('ul.side-nav li a label',stack).unwrap();$('ul.side-nav li a hr',stack).unwrap().parent().addClass('divider').html('');});
